...
import { findByIdentifier, verifyPassword } from "../../src/users/users.js";
...
describe("POST /users", () => {
  test("creates the account and answers with the user, never the hash", async () => {
    const response = await register({ email: "a@x.com", username: "ann", password: "a-good-password" });
  ...
  test("stores a hash, not the password", async () => {
    await register({ email: "a@x.com", username: "ann", password: "a-good-password" });

    const stored = await findByIdentifier(testPool(), "a@x.com");
  ...
  test("normalises the email so Bob@X.com and bob@x.com are one account", async () => {
    expect((await register({ email: "Bob@X.com", username: "bob", password: "a-good-password" })).json().email)
      .toBe("bob@x.com");

    const second = await register({ email: "bob@x.com", username: "bobby", password: "another-password" });
    expect(second.statusCode).toBe(409);
  });

  test("a taken email is 409, and the account is not overwritten", async () => {
    await register({ email: "a@x.com", username: "ann", password: "the-first-password" });
    const response = await register({ email: "a@x.com", username: "anna", password: "the-second-password" });
  ...
  test("rejects bad input before it reaches the database", async () => {
    const cases: [unknown, string][] = [
      [{ email: "not-an-email", username: "ann", password: "a-good-password" }, "email"],
      [{ email: "a@x.com", username: "ann", password: "short" }, "password"],
      [{ email: "a@x.com", username: "ann" }, "password"],
      [{ username: "ann", password: "a-good-password" }, "email"],
      [{ email: "a@x.com", password: "a-good-password" }, "username"],
      [{ email: "a@x.com", username: "-nope", password: "a-good-password" }, "username"],
    ];
