...
describe("authorisation", () => {
  ...
  test("a forged or revoked cookie is 401, not a crash", async () => {
    ...
    await app.inject({ method: "DELETE", url: "/sessions", ...asUser(user) });

    // Same cookie, now revoked — this is the check that makes logout mean
    // something. A JWT would still be accepted here.
    expect((await app.inject({ method: "GET", url: "/decks", ...asUser(user) })).statusCode).toBe(401);
    expect((await app.inject({
      method: "GET", url: "/decks", headers: { cookie: `${SESSION_COOKIE}=made-up` },
    })).statusCode).toBe(401);
  });
